...
REGEX_PATTERNS = {
    ...
    # Alias for short-name queries
    "founder_info": [r"\bramin\b", r"\bmohammad ramin\b", r"\bkhanzada\b"]
}
...
def train_ml_classifier():
    if not FAQ_JSON_PATH.exists():
        print(f" {FAQ_JSON_PATH} not found. Cannot train ML classifier.")
        return
    ...
    first_entry = faq_data[X[0]]
    if isinstance(first_entry, str):
        y = [faq_data[q] for q in X]
    elif isinstance(first_entry, dict) and "intent" in first_entry:
        y = [faq_data[q]["intent"] for q in X]
    else:
        print(" FAQ format unrecognized. Cannot train ML classifier.")
        return
    ...
    print(f" ML classifier trained and saved to {MODEL_PATH} / {VECT_PATH}")
...
def load_ml_classifier():
    ...
    if MODEL_PATH.exists() and VECT_PATH.exists():
        ...
        print(" ML classifier loaded into memory.")
    else:
        print(" ML classifier not found. Training a new model...")
        train_ml_classifier()
        if MODEL_PATH.exists() and VECT_PATH.exists():
            with open(MODEL_PATH, "rb") as f:
                clf_model = pickle.load(f)
            with open(VECT_PATH, "rb") as f:
                tfidf_vectorizer = pickle.load(f)
            print(" ML classifier trained and loaded.")
    ...
    for intent, patterns in REGEX_PATTERNS.items():
        for pattern in patterns:
            if re.search(pattern, cleaned):
                # Special case: Ramin/founder
                if intent == "founder_info":
                    intent_name = "who is Mohammad Ramin Khanzada?"
                    return {"text": user_input, "cleaned": cleaned, "intent": intent_name, "confidence": 0.85,
                            "tokens": [t.text for t in doc], "language": doc.lang_ if hasattr(doc, "lang_") else "en"}
